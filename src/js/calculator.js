 ...
 * Version: 7.1 (Syntax Fix)
 * Описание: Полностью автономный скрипт для калькулятора.
 */
(function () {
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		// Если на странице нет блока с калькулятором, ничего не делаем
		if (!simulatorNode) {
			return;
		}

		// Если библиотека Chart.js не подключена на странице, показываем ошибку и выходим
		...
		// Если все на месте, запускаем основную логику калькулятора
	...
	function runCalculatorLogic(simulatorNode) {
		...
		const els = {
			...
			initialPartnersValue: simulatorNode.querySelector(
				"#initialPartnersValue"
			),
			partnerDuplicationValue: simulatorNode.querySelector(
			...
			modeButtons: simulatorNode.querySelectorAll(".mode-switch-btn"),
			simulatorSubtitle: simulatorNode.querySelector("#simulatorSubtitle"),
			priceLabel: simulatorNode.querySelector("#priceLabel"),
			initialPartnersLabel: simulatorNode.querySelector(
				"#initialPartnersLabel"
			),
			duplicationLabel: simulatorNode.querySelector("#duplicationLabel"),
		};

		if (!els.price || !els.chartCtx) {
			console.error(
				"Calculator Error: One of the essential HTML elements is missing."
			);
			return;
		}
		...
		function runSimulation(modelConfig, mode) {
			...
			if (mode === "author") {
				...
				const authorIncome = totalRevenue - totalPayout;
		...
		window.setSimulatorMode = function (newMode) {
			...
			if (newMode === "author") {
				...
				els.classic.incomeLabel.innerText = "Ваш чистый доход";
				els.sethubble.incomeLabel.innerText = "Ваш чистый доход";
			} else {
				els.simulatorSubtitle.innerText =
				...
				els.classic.incomeLabel.innerText = "Ваш комиссионный доход";
				els.sethubble.incomeLabel.innerText = "Ваш комиссионный доход";
			}
		...
		window.renderSimulator = function () {
			...
			let incomeFactor = "почти столько же";
			if (
			...
			} else if (classicResults.income === 0 && sethubbleResults.income > 0) {
				incomeFactor = "неизмеримо";
			}
		...
		updateSimulatorUI();
		window.renderSimulator();
	} // ✅ ИСПРАВЛЕНО: Закрывающая скобка для runCalculatorLogic
})(); // ✅ ИСПРАВЛЕНО: Закрывающая скобка для самовызывающейся функции
