 ...
 * Version: 5.0 (Non-critical, On-demand Logic)
 * Описание: Этот скрипт содержит всю логику для калькулятора.
 * Он загружается асинхронно и инициализируется только при необходимости.
 */

function initSimulator() {
    // --- 1. Динамическая загрузка Chart.js ---
    ...
    // Запускаем основную логику калькулятора только после загрузки Chart.js
    loadChartLibrary(runCalculatorLogic);
}
...
function runCalculatorLogic() {
    ...
    window.renderSimulator = function () {
        ...
        const styles = getComputedStyle(document.getElementById('simulator'));
...
// Отправляем сигнал, что этот скрипт загружен
document.dispatchEvent(new Event('calculatorLoaded'));
